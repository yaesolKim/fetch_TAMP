...
if __name__ == "__main__":
    ...
    for b in range(0, block_no):
        block_path = os.path.join(PATH, 'models', 'block', 'model'+str(b)+'.urdf')
        block_name = 'block'+str(b)
        block_pose = Pose(position=Point(x=0.57+b*0.1, y=-0.05, z=0.7))
        time.sleep(1.0)
...
#    grasping_client.stow()
    ...
    for x in range(0, block_no): # for x in range(block_no, block_no+1):
