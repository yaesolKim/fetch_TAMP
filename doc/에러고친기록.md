...
### 1.
문제: 아래 링크의 질문처럼 경로가 ~~~//~~~ 로 나온다. 무빗 플래닝 에러 뜨는것도 이것 때문이라고 예상된다.

https://answers.ros.org/question/250630/the-kinematics-plugin-manipulator-failed-to-load/

해결방법: catkin workspace의 src와 devel 파일을 삭제하고 다시 빌드한다...


### 2.
문제: fetch가 object를 grasp(pick)한 후 place 하는 과정에서 물체를 놓친다 (물체가 날라간다)

시도해본 방법:
1. object의 friction coefficient를 높인다: 30 -> 100
-> 소용 없음

2. fetch_description/robot.fetch.urdf 파일에 명세된 fetch arm의 joint velocity들을 낮춘다
-> 덜 놓치지만 그래도 놓침,,, 플래닝 실패 에러만 더 많이 생긴다.
-> 파생된 문제로, 문제 3이 발생한다. 

3. grap 한 뒤에 물체를 그리퍼에 붙여버린다
-> 해결!

해결방법:
gazebo_grasp_fix 플러그인 소스를 다운로드하여 빌드한 뒤,
fetch urdf에 해당되는 fetch_gazebo/robots/fetch.gazebo.xacro 코드 하단에 플러그인 쓰도록 코드를 추가한다.


### 3.
문제: Trajectory not excuted within time limit 에러

시도해본 방법: fetch_moveit_config/launch/trajectory_execution.launch.xml의 duration margin을 높인다
-> 해결 못함. 소용 없음. joint velocity를 다시 높이니 에러 발생 안한다.


### 4.
문제: gazebo_grasp_fix 플러그인을 쓰는데 grasp 후에도 물체가 종종 떨어진다

해결방법: 플러그인 사용하는 코드에서 release tolerance를 수정한다
0.005 -> 쉽게 떨어짐
0.05 -> 계속 붙어만 다님
0.008 -> 적당함
...
### 5.
문제: gazebo REST error

해결 방법: home/.ignition/fuel/config.yaml 고친다.
